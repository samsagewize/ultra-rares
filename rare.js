...
const rareHolderCountElement = document.querySelector('[data-rare-holder-count]');
...
async function refreshRareMarket() {
  try {
    ...
    rareHolderCountElement.textContent = market.holderCount === null ? 'Unavailable' : Number(market.holderCount).toLocaleString('en-US');
    ...
    rareHolderCountElement.textContent = 'Unavailable';
...
function renderRareTransfers(transfers, newBuyHash = null, newTradeHashes = new Set(), currentLatestBuyHash = null) {
  ...
  const rows = transfers.map((transfer) => {
    ...
    if (transfer.side === 'liquidity') {
      const liquidityBadge = document.createElement('em');
      liquidityBadge.textContent = '+LP';
      icon.append(liquidityBadge);
    }
    if (transfer.side === 'auction') {
      const auctionBadge = document.createElement('em');
