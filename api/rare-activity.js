...
const PONS_MARKET='0xbc5c9e55a85675851174f3bc154bca1b8da925f5';
...
module.exports=async function handler(request,response){if(request.method!=='GET')return response.status(405).json({error:'Method not allowed'});try{
  ...
  const address=(topic='')=>`0x${topic.slice(-40)}`;const transfers=recent.map(log=>{const from=address(log.topics[1]);const to=address(log.topics[2]);return {hash:log.transactionHash,logIndex:Number(BigInt(log.logIndex)),from,to,value:BigInt(log.data).toString(),decimals:18,side:from.toLowerCase()===PONS_MARKET?'buy':to.toLowerCase()===PONS_MARKET?'sell':'transfer',timestamp:new Date(timestamps.get(log.blockNumber)||Date.now()).toISOString(),url:`${EXPLORER}/tx/${log.transactionHash}`};});
