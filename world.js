(() => {
  ...
  function draw(){
    ...
    const px=Math.round(player.x-camera.x),py=Math.round(player.y-camera.y);const walking=keys.has('arrowleft')||keys.has('arrowright')||keys.has('arrowup')||keys.has('arrowdown')||keys.has('w')||keys.has('a')||keys.has('s')||keys.has('d');const step=walking?Math.sin(performance.now()/90)*5:0;
    ctx.fillStyle='#000';ctx.fillRect(px-22,py+5+Math.max(0,step),13,24);ctx.fillRect(px+9,py+5+Math.max(0,-step),13,24);ctx.fillStyle='#b6ff00';ctx.fillRect(px-18,py+9+Math.max(0,step),6,15);ctx.fillRect(px+12,py+9+Math.max(0,-step),6,15);
    ctx.save();ctx.imageSmoothingEnabled=false;ctx.beginPath();ctx.roundRect(px-34,py-54,68,68,28);ctx.clip();ctx.drawImage(player.image,px-34,py-54,68,68);ctx.restore();ctx.strokeStyle='#000';ctx.lineWidth=4;ctx.beginPath();ctx.roundRect(px-34,py-54,68,68,28);ctx.stroke();ctx.fillStyle='#000';ctx.fillRect(px-43,py+33,86,20);ctx.fillStyle='#b6ff00';ctx.font='900 10px Arial';ctx.textAlign='center';ctx.fillText(player.name.toUpperCase().slice(0,14),px,py+47);
  }
